...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (btnAdicionarImovel) {
        btnAdicionarImovel.addEventListener('click', function () {
            ...
            newBlock.querySelectorAll('[data-tab]').forEach(el => {
                el.setAttribute('data-tab', el.getAttribute('data-tab').replace(/_0$/, `_${newIndex}`));
            });
            ...
            // Garante que o novo imóvel sempre comece com a Aba 1 (Identificação) ativa
            const tabLinks = newBlock.querySelectorAll('.tab-link');
            const tabPanes = newBlock.querySelectorAll('.tab-pane');
